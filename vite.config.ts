    ...
    react(),
    {
        ...
        configureServer(server) {
            ...
            const needsSetup =
              ...
              (!provider ||
                (!OAUTH_PROVIDERS.has(provider) &&
                  ...
                  !hasProviderKey));
            let statusPayload = { installed, binPath, version, configured, defaultModel, provider, providerKeyName, hasProviderKey, needsSetup, envPath };
            // Override with remote bridge status if Hermes isn't installed locally
            if (!installed) {
              try {
                const remoteFile = join(homedir(), ".claude-os", "hermes-remote");
                if (existsSync(remoteFile)) {
                  const remoteUrl = readFileSync(remoteFile, "utf-8").trim();
                  if (remoteUrl) {
                    statusPayload = { installed: true, binPath: remoteUrl, version: "remote (VM)", configured: true, defaultModel: "hermes-vm", provider: "remote", providerKeyName: null, hasProviderKey: true, needsSetup: false, envPath };
                  }
                }
              } catch { /* ignore */ }
            }
          ...
          server.middlewares.use("/__hermes_chat", async (req, res, next) => {
            ...
            // Proxy to remote Hermes bridge (VM) if configured
            {
              let remoteUrl: string | undefined;
              try {
                const f = join(homedir(), ".claude-os", "hermes-remote");
                if (existsSync(f)) remoteUrl = readFileSync(f, "utf-8").trim().replace(/\/$/, "") || undefined;
              } catch { /* ignore */ }
              if (!remoteUrl) remoteUrl = process.env.HERMES_REMOTE_URL?.trim().replace(/\/$/, "");
              if (remoteUrl) {
                ...
                const { request: mkReq } = await import("node:https");
                const u = new URL(`${remoteUrl}/chat`);
                const bs = JSON.stringify({ prompt, ...(sessionId && { sessionId }), ...(hasToolsets && { toolsets }), yolo, graph });
                const pr = mkReq({ hostname: u.hostname, port: Number(u.port) || 443, path: u.pathname || "/chat", method: "POST", headers: { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(bs) } }, (rs) => {
                  rs.on("data", (c: Buffer) => { try { res.write(c); } catch { /* ignore */ } });
                  rs.on("end", () => { try { res.end(); } catch { /* ignore */ } });
                  rs.on("error", (e: Error) => { try { res.write(`event: error\ndata: ${e.message}\n\n`); res.end(); } catch { /* ignore */ } });
                });
                pr.on("error", (e: Error) => { try { res.write(`event: error\ndata: Remote Hermes: ${e.message}\n\n`); res.end(); } catch { /* ignore */ } });
                req.on("close", () => { try { pr.destroy(); } catch { /* ignore */ } });
                pr.write(bs); pr.end();
                return;
              }
            }
